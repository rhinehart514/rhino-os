...
Features have a journey position (entry/core/leaf) computed from the product topology graph. This adjusts how much delivery vs. craft matters:

| Position | Delivery multiplier | Rationale |
|----------|-------------------|-----------|
| **entry** | 1.2x | Entry features block ALL downstream value. If install is broken, nobody reaches scoring. Delivery is paramount — craft is secondary. |
| **core** | 1.1x | Core features are depended on by other features. Reliability matters more than polish. |
| **leaf** | 1.0x | Leaf features affect only their direct users. Standard weighting. |

**How the multiplier works:** The delivery weight in the formula `delivery * 0.60 + craft * 0.40` becomes `delivery * (0.60 * multiplier)`. For entry features: `delivery * 0.72 + craft * 0.28`. This makes delivery count MORE for features that gate user value.

**Example:** An entry feature with delivery=55, craft=70 scores:
- Without journey weight: `55*0.60 + 70*0.40 = 61`
- With 1.2x entry weight: `55*0.72 + 70*0.28 = 59` — the low delivery pulls harder because it gates everything

Run `bash scripts/journey-weight.sh <feature>` to get the position and multiplier. The topology is generated by `skills/shared/product-topology.sh` and cached at `.claude/cache/topology.json`.
