---
name: strategist
description: "Use when you need market intelligence, honest diagnosis, bet scoring, or competitive response"
...
## When to use /strategist vs other commands

| Command | Question | Scope |
|---------|----------|-------|
| `/strategist` | What should we build given the market? Where are we honestly? | Outward + inward |
...
`/strategist` is the thinking layer. `/ideate` is the action layer. `/plan` is the execution layer. `/eval` is the measurement layer.
...
Combines old `/strategy` diagnosis with market intelligence. Reads everything:
...
── strategist ──────────────────────────────────
  ...
  /strategist bet [idea]   score a product bet
  /strategist market       deep landscape dive
...
`/strategist market` builds and updates `.claude/cache/market-context.json`:
...
All modes read from this cache. `/strategist market` refreshes it.
...
- No market-context.json: read base model, run inline WebSearch, suggest `/strategist market`
